...
const updateGroupCandles = (ticker) => {
  ...
  if (Array.isArray(ticker[1][0])) {
    ...
    let candles = ticker[1].filter((tick) => {
...
export const getCandlesGroupDataEpic = (action$, state$) => {
  return action$.ofType(FETCH_CANDLES_DATA_PORTFOLIO_START)
    .mergeMap((action) => {
      return Observable.concat(
        ...
        candlesGroupWebsocket$(
          ...
          .takeUntil(
            action$.ofType(FETCH_CANDLES_DATA_PORTFOLIO_STOP)
          )
          .filter(val => {
            return val[1] !== "hb"
          }
...
const getTickers$ = (relay, networkId, tickersString) => {
  ...
  return Observable
    .fromPromise(exchange.getTickers(tickersString))
  // .catch((error) => {
  //   console.log(error)
    ...
    .switchMap((action) => {
      return Observable
        ...
        .exhaustMap(() => {
          ...
          const tickersString = utils.ethfinexTickersToArray(currentState.transactionsDrago.selectedDrago.assets).toString()
          return getTickers$(
            action.payload.relay,
            action.payload.networkId,
            tickersString)
