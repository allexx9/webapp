...
export default class InfoTable extends Component {
  ...
  renderColumn = (row) =>{
    ...
    return row.map((cell, index) => {
      ...
      const rowCell = index == 0 
        ? <TableRowColumn className={columnsStyle[index]} style={this.compactStyle} key={key}>{cell}</TableRowColumn>
        : <TableRowColumn className={columnsStyle[index]} style={this.compactStyle} key={key}>{cell}</TableRowColumn>
      return rowCell
    })
  }
  ...
  renderRow = (rows) => {
    return rows.map((row, index) => {
      return (<TableRow hoverable={false} key={'infoTableRow'+index} style={this.compactStyle}>
