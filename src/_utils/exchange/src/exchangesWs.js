...
export const getTicker = {
  ERCdEX: (networkId = 1, baseTokenAddress, quoteTokenAddress) => {
    const websocket = new ReconnectingWebSocket(SupportedExchanges.ERCdEX.ws[NETWORKS_ID[networkId]])
    websocket.addEventListener('open', () => {
      ...
      try {
        ...
        websocket.send(`sub:pair-order-change/${quoteTokenAddress}/${baseTokenAddress}`)  
          ...
          throw new Error(`Error connectiong to ${SupportedExchanges.ERCdEX.ws[NETWORKS_ID[networkId]]}`);
      }
    });
    return websocket
  },
