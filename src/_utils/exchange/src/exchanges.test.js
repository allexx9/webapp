...
describe("get historical price data from exchange REST API", () => {
  ...
  networks.map((network, key) => {
    it(`${ERCdEX} get historical price data for ${network} success`, () => {
      const networkId = key.toString()
      const result = getHistoricalPricesData[ERCdEX](networkId, baseTokenAddress, quoteTokenAddress, startDate)
      expect(result)
      .toEqual(
        {
          method: 'POST',
...
describe("get orders from exchange REST API", () => {
  ...
  networks.map((network, key) => {
    it(`${ERCdEX} get orders for ${network} success`, () => {
      const networkId = key.toString()
      const result = getOrders[ERCdEX](networkId, baseTokenAddress, quoteTokenAddress)
      expect(result)
      .toEqual(
        {
          method: 'GET',
